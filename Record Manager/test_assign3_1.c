...
Record *testRecord1(Schema *schema, int empID, char *empName, int empAge,float empSalary,bool Results);
Schema *testSchema1(void);
Record *fromTestRecord1(Schema *schema, TestRecord1 in);
...
{
  Schema *result;
  char *colName[] = { "empID", "empName", "empAge","empSalary","Results" };
  DataType dt[] = { DT_INT, DT_STRING ,DT_INT,DT_FLOAT, DT_BOOL };
  int sizes[] = { 0, 10, 0,0,0 };
  int keys[] = {0};
  int i;
  char **cpyColName = (char **) malloc(sizeof(char*) * 5);
  DataType *cpyDatatype = (DataType *) malloc(sizeof(DataType) * 5);
  int *cpySize = (int *) malloc(sizeof(int) * 5);
  int *cpyKey = (int *) malloc(sizeof(int));
  int length=0;

  for(i = 0; i < 5; i++)
    {
      length=strlen(colName[i]);
      cpyColName[i] = (char *) malloc(length+1);
      strcpy(cpyColName[i], colName[i]);
    }
  memcpy(cpyDatatype, dt, sizeof(DataType) * 5);
  memcpy(cpySize, sizes, sizeof(int) * 5);
  memcpy(cpyKey, keys, sizeof(int));

  result = createSchema(5, cpyColName, cpyDatatype, cpySize, 1, cpyKey);

  return result;
}

Record *
fromTestRecord1 (Schema *schema, TestRecord1 in)
{
  return testRecord1(schema, in.empID, in.empName, in.empAge,in.empSalary,in.Results);
}

Record *
testRecord1(Schema *schema, int empID, char *empName, int empAge,float empSalary,bool Results)
{
  Record *result;
  Value *value;
  ...
  MAKE_VALUE(value, DT_INT, empID);
  TEST_CHECK(setAttr(result, schema, 0, value));
  freeVal(value);

  MAKE_STRING_VALUE(value, empName);
  TEST_CHECK(setAttr(result, schema, 1, value));
  freeVal(value);

  
MAKE_VALUE(value, DT_INT, empAge);
  TEST_CHECK(setAttr(result, schema, 2, value));
  freeVal(value);

 
MAKE_VALUE(value, DT_FLOAT, empSalary);
  TEST_CHECK(setAttr(result, schema, 3, value));
  freeVal(value);

 MAKE_VALUE(value, DT_BOOL, Results);
...
testInsertRecords(void)
{
  ...
  TestRecord1 inserts[] = { 
    ...
    {3, "JOHN", 40,85.9,TRUE},
    {4, "RAM", 50,60.8,TRUE},
    {5, "RADHA", 19,88.6,FALSE},
    {6, "MILI", 15,55.5,FALSE},
    {7, "JACK", 23,99.4,FALSE},
    {8, "DANIEL", 33,72.3,TRUE},
    {9, "PETER", 43,83.2,TRUE},
    {10, "ROSE", 10,92.1,TRUE},
  };
  TestRecord1 realInserts[10000];
  TestRecord1 updates[] = {
    {11, "DONALD", 51,72.3,TRUE}
  };
  int numInserts = 10000, i;
  int recID = 11;
  Record *r;
  RID *rids;
  Schema *schema;
  testName = "test creating a new table and inserting 10000 records then updating record from rids[11]";
  schema = testSchema1();
  rids = (RID *) malloc(sizeof(RID) * numInserts);
  ...
  for(i = 0; i < numInserts; i++)
    {
      realInserts[i] = inserts[i%10];
      realInserts[i].empID = i;
      r = fromTestRecord1(schema, realInserts[i]);
      TEST_CHECK(insertRecord(table,r)); 
      rids[i] = r->id;
    }
  TEST_CHECK(closeTable(table));
  ...
  for(i = 0; i < numInserts; i++)
    {
      RID rid = rids[i];
      TEST_CHECK(getRecord(table, rid, r));
      ASSERT_EQUALS_RECORDS(fromTestRecord1(schema, realInserts[i]), r, schema, "compare records");
    }
  
  r = fromTestRecord1(schema, updates[0]);
  r->id = rids[recID];
  TEST_CHECK(updateRecord(table,r));
  TEST_CHECK(getRecord(table, rids[recID], r)); 
  ASSERT_EQUALS_RECORDS(fromTestRecord1(schema, updates[0]), r, schema, "compare records");
...
testInsertRecords(void)
{
  RM_TableData *table = (RM_TableData *) malloc(sizeof(RM_TableData));
  TestRecord1 inserts[] = { 
    {1, "aaaaaaaaaa", 30,78.6,TRUE}, 
    {2, "bbbbbbbbbb", 40,89.0,TRUE},
    {3, "cccccccccc", 56,34.3,FALSE},
    {4, "dddddddddd", 63,54.8,TRUE},
    {5, "eeeeeeeeee", 71,21.7,TRUE},
    {6, "ffffffffff", 32,65.1,FALSE},
    {7, "gggggggggg", 43,76.2,TRUE},
    {8, "hhhhhhhhhh", 54,87.5,TRUE},
    {9, "iiiiiiiiii", 18,64.6,FALSE},
    {10, "jjjjjjjjj", 27,73.4,TRUE},
  };
  TestRecord1 realInserts[10000];
  TestRecord1 updates[] = {
    {11, "mmmmmmmmmm", 60,78.3,TRUE}
  };
  int numInserts = 10000, i;
  int recID = 11;
  Record *r;
