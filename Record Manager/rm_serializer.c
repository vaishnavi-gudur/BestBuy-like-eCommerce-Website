...
// prototypes
// commented below as it has reference in record_mgr.c and has been redefined in tables.h
...
char * 
serializeSchema1(Schema *schema)
{
  int i;
  VarString *result;
  MAKE_VARSTRING(result);

  APPEND(result, "%i", schema->numAttr);
APPEND_STRING(result,";");
  for(i = 0; i < schema->numAttr; i++)
    {
      APPEND(result,"%s%s: ", (i != 0) ? ", ": "", schema->attrNames[i]);
  ...
  APPEND_STRING(result,";");
 APPEND(result,"%d",schema->keySize);
APPEND_STRING(result,";");

  for(i = 0; i < schema->keySize; i++)
    APPEND(result, "%s%d", ((i != 0) ? ", ": ""), schema->keyAttrs[i]); 
